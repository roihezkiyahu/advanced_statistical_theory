...
\\
loglik(y,p) = d(y) +(n-\sum_{j=1}^nI(y_i<k)) ln(1-\sum_{m=1}^{k-1}p_m) + \sum_{i=1}^{k-1}[ln(p_i){\sum_{l=1}^n{I(y_l = i)}}] = d(y)+ nln(1-\sum_{m=1}^{k-1}p_m)+\sum_{i=1}^{k-1}[ln\left(\frac{p_i}{1-\sum_{m=1}^{k-1}p_m}\right){\sum_{l=1}^n{I(y_l = i)}}]
\\
...
\\
\textrm{thus the joint distribution of } (X_1,X_2,X_3) \textrm{ belongs to the 2 paramater exponential family with the minimal and complete sufficent statistic } (T_1(X),T_2(X))
$$
